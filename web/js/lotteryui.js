...
/*
Plugin: trigger selection
*/
$.fn.selected = function(method, options) {
...
var Lottery = (function() {
  ...
  var init = function() {
    ...
    $container.css("min-height", itemHeight + 'px');
    speed = itemWidth / 3 | 0;
...
var Nav = (function() {
  ...
  var load = function(hash, url) {
    ...
    var html  = $tmpl.size() ? $tmpl.html().trim() : "";
  ...
  var mvto = function(hash) {
    ...
    if (!loaded) {
      //Is it a template?
      var $tmpl = $("style[data-hash='" + hash.replace("#", '') + "']");
      $tmpl.size() && Lottery.add(hash, $tmpl.html());
      //Finally, try to navigate to the slide
      Lottery.mvto(hash);
    }
  };
